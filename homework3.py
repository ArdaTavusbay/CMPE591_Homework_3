...
class Hw3Env(environment.BaseEnv):
    ...
    def reward(self):
        ...
        r_ee_to_obj = -0.1 * d_ee_to_obj  # bonus for getting closer to object
        r_obj_to_goal = -0.2 * d_obj_to_goal  # bonus for moving object to goal
...
if __name__ == "__main__":
    ...
    plt.plot(rews, label='Episode Reward')
    
    window_size = 50
    if len(rews) >= window_size:
        moving_avg = np.convolve(rews, np.ones(window_size)/window_size, mode='valid')
        plt.plot(range(window_size - 1, num_episodes), moving_avg, label='Moving Average (50 episodes)', color='red')
    
    plt.xlabel('Episode')
    plt.ylabel('Cumulative Reward')
    plt.title('Vanilla Policy Gradient (REINFORCE) Training Rewards')
    plt.legend()
    plt.grid(True)
